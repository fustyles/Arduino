...
int n2=0;    //第二次擊掌狀態
int n3=0;    //第三次擊掌狀態
int bin1=0;    //紀錄輸入第一位數字
int bin2=0;    //紀錄輸入第二位數字
int bin3=0;    //紀錄輸入第三位數字
int lim1=520;  //產生聲音下限基準值
int lim2=700;  //產生聲音大小聲基準值 
 ...
 if (s1==0)          //若偵測到聲音產生低電位訊號
  {
    ...
    { 
      ...
      if (n1==0)   
      {
        ...
        if (s2>=lim2)
          bin1=1;    //音量>=基準值，二進制第一位數字設為1
        else
          bin1=0;    //音量<基準值，二進制第一位數字設為0
        n2=0;
        n3=0;
        t=0;
        delay(200); 
